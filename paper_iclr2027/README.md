...
- Statements begin on page 9, references on page 10, and appendices on page 13.
- The complete PDF is 36 pages.
- Table 1 compares seven configurations over five benchmarks. CoMem and CoMem (without LoRA) both use j=12, specified in the caption. The benchmark detail tables use the same no-adapter operating point.
...
The architecture is on page 5, the teaser on page 2, and the complete source-scaling figure on page 26. Table 1 compares seven methods over five benchmarks. Both CoMem variants use j=12, stated in the caption; their row labels are CoMem and CoMem (without LoRA). Benchmark detail tables use the same j=12 no-adapter results. Table 2 compares full-source Dense with CoMem on RTX 5090 under the 28 GB allocator cap, alongside the B300 full-context comparison and B200 pipeline. Same-pack local replay remains a separate appendix diagnostic.
