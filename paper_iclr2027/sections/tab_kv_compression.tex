...
\bottomrule
\end{tabular}
\caption{Full-prefill retained-KV timing on H20, median of three with compression and 64-token generation. These methods prefill the whole source before eviction. The hardware differs from the B200 CoMem pipeline, so no cross-table speedup is inferred.}
\label{tab:kv-timing}
\end{table}
