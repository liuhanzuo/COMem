...
On the common answerable items, MidCache scores 48.64 and KV-Direct 43.83. Their paired $+4.81$-point difference has a 95\% item-bootstrap interval of $[2.34,7.34]$ using 10,000 resamples and seed 1234. Resampling the ten conversations instead gives $[1.27,8.32]$; eight of ten observed conversation differences favor MidCache. The cluster interval addresses dependence but does not create additional independent conversations, and its tail fraction is not reported as a calibrated hypothesis-test $p$ value.

An independent July 26, 2026 audit applies DeepSeek-V3 to a stratified 200-item paired subset. Agreement with GPT-4o is 0.81, with $\kappa=0.626$. GPT-4o scores MidCache/KV-Direct at 36.5/32.5 and DeepSeek-V3 at 56.0/49.0. The ordering persists on this subset, although absolute calibration differs.

Descriptive per-method intervals use 1,000 item-bootstrap resamples with seed 1234: full-set Judge is $[36.20,40.58]$ for MidCache and $[32.48,36.71]$ for KV-Direct; answerable-item Judge is $[46.04,50.98]$ and $[41.23,46.30]$; F1 is $[8.60,9.72]$ and $[8.51,9.61]$. These unpaired intervals do not replace the paired cluster comparison. The six-length LongEval mean has 95\% interval $[69.33,76.33]$; the main table instead uses the five-length mean of 69.0.

\subsection{Prompt and training-run sensitivity}
\input{sections/tab_prompt_runs}
Native chat formatting changes MidCache's LoCoMo Judge from 38.27 to 37.76 and KV-Direct's from 34.59 to 38.22, reversing their order. Lexical metrics change more strongly: MidCache F1/substring accuracy goes from 9.15/23.36 to 19.51/28.65, and KV-Direct F1 goes from 9.02 to 40.06. The no-chat semantic comparison is therefore protocol-specific. No matched native-chat evaluation is available for MemoryLLM, so no corresponding sensitivity conclusion is drawn.
